...
class BaseNeuralNetwork(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def _validate(self):
        ...
        if self.algorithm not in ['hill_climb', 'random_hill_climb', 'simulated_annealing',
                                  'genetic_alg', 'gradient_descent']:
            raise Exception("""Algorithm must be one of: 'hill_climb', 'random_hill_climb',
    ...
    def fit(self, X, y=None, init_weights=None):
        ...
        if self.algorithm == 'hill_climb':
            fitted_weights = None
            loss = np.inf

            fitness_curve = []
            # Can't use restart feature of hill_climb function, since
            ...
            for _ in range(self.restarts + 1):
                ...
                if self.curve:
                    current_weights, current_loss, current_fitness_curve = \
                        hill_climb(problem,
                            max_iters=self.max_iters,
                            restarts=0,
                            init_state=init_weights,
                            curve=self.curve)
                    fitness_curve += list(current_fitness_curve)
                else:
                    current_weights, current_loss = hill_climb(
                        problem,
                        max_iters=self.max_iters,
                        restarts=0,
                        init_state=init_weights,
        ...
        elif self.algorithm == 'random_hill_climb':
            fitted_weights = None
            loss = np.inf
        ...
        if self.curve:
            self.fitness_curve = np.array(fitness_curve)
