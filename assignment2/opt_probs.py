        ...
        for i in range(1, self.length):
            for j in range(self.max_val):
                ...
                if not len(subset):
                    ...
                    # Check if noise argument is non-zero
                    if self.noise != 0:
...
class ContinuousOpt(OptProb):
    ...
    def reproduce(self, parent_1, parent_2, mutation_prob=0.1):
        ...
        # Reproduce elitist child
        elitist_child_fitness = None
        for _n in range(self.length+1):
            child = np.array([0]*self.length)
            child[0:_n] = parent_1[0:_n]
            child[_n:] = parent_2[_n:]

            child_fitness = self.eval_fitness(child)
            if elitist_child_fitness == None or child_fitness > elitist_child_fitness:
                elitist_child_fitness = child_fitness
                elitist_child = child
        ...
        for i in mutate:
            elitist_child[i] = np.random.uniform(self.min_val, self.max_val)

        return elitist_child
...
class TSPOpt(DiscreteOpt):
    ...
    def reproduce(self, parent_1, parent_2, mutation_prob=0.1):
        ...
        if len(mutate) > 0:
            ...
            temp = np.copy(elitist_child)

            for i in range(len(mutate)):
                elitist_child[mutate[i]] = temp[mutate_perm[i]]
